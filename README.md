...
该能力已拆分为独立扩展 **雷动远程资源（leidong-sftp）**：远程目录浏览与文件传输、本地与远程文件比较/同步、备份上传、保存自动上传、交互式远程终端。命令 ID 与配置键完全沿用 `leidong-tools.sftp.*` / `leidong-tools.remoteTerminal.*`，已保存的 `.vscode/sftp.json` 与设置无需迁移；自动上传目标与远程终端历史因扩展存储隔离需重新选择一次。

### Copilot Chat 自定义端点（DeepSeek / MiMo）

该能力已拆分为独立扩展 **雷动模型配置（leidong-model-config）**。安装后从命令面板运行“配置自定义模型”，即可可视化管理模型列表、自定义模型名称、端点和 API Key（密钥保存在 VS Code 安全存储）。旧版的 `leidong-tools.copilot.*` 设置会在新扩展首次激活时自动迁移；API Key 因扩展安全隔离需要重新设置一次。
