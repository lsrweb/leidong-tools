...
export async function prepareChatRequest({
	...
	const endpoint = modelDef?.endpoint ?? 'deepseek';
	const credentialSet = endpoint === 'mimo' && getMiMoAccessMode() === 'tokenPlan' ? 'mimo-token-plan' : endpoint;
	const apiKey = await authManager.getApiKey(credentialSet);
	if (!apiKey) {
	...
	const baseRequest: DeepSeekRequest = {
		model: getApiModelId(modelInfo.id, endpoint),
	...
	const forceNoneThinking = endpoint === 'deepseek' &&
