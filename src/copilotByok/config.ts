 ...
 */
export function getBaseUrl(endpoint: 'deepseek' | 'mimo' = 'deepseek'): string {
	const config = vscode.workspace.getConfiguration(CONFIG_SECTION);
	if (endpoint === 'mimo') {
		const accessMode = getMiMoAccessMode();
		if (accessMode === 'tokenPlan') {
			const region = config.get<'cn' | 'sgp' | 'ams'>('mimoTokenPlanRegion', 'cn');
	...
	return config.get<string>('deepseekBaseUrl') || 'https://api.deepseek.com';
}
 ...
 */
export function getApiModelId(vscodeModelId: string, endpoint: 'deepseek' | 'mimo' = 'deepseek'): string {
	const config = vscode.workspace.getConfiguration(CONFIG_SECTION);
	const overrides = config.get<Record<string, string>>(endpoint === 'mimo' ? 'mimoModelIdOverrides' : 'deepseekModelIdOverrides');
