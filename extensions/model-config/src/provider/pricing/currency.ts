...
const CACHE_KEY = 'leidong-models.balanceCurrency.cache';
...
export class BalanceCurrencyResolver {
	...
	getDisplayCurrency(baseUrl: string, apiKeyModelId: string): PricingCurrency | undefined {
		baseUrl = normalizeBaseUrl(baseUrl);
		if (!isOfficialDeepSeekBaseUrl(baseUrl)) {
			// Non-official endpoints have no balance lookup; fall back to the
			// display locale so any configured pricing still renders.
	...
	refreshInBackground(baseUrl: string, apiKeyModelId: string): void {
		if (this.inFlight || !this.needsRefresh(baseUrl)) {
			return;
		}

		const controller = new AbortController();
		const generation = this.generation;
		const refresh = this.refreshFromBalance(baseUrl, apiKeyModelId, controller, generation)
			.catch((error) => {
	...
	private needsRefresh(baseUrl: string): boolean {
		baseUrl = normalizeBaseUrl(baseUrl);
		if (!isOfficialDeepSeekBaseUrl(baseUrl)) {
			return false;
		}
	...
	private async refreshFromBalance(
		baseUrl: string,
		...
		const apiKey = await this.authManager.getApiKey(apiKeyModelId);
		if (!apiKey) {
			return;
		}
